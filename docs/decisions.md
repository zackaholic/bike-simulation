...
## R2026-06-21 — Webview redesign around the ride lens

**Decision**: Rebuild the inspector so the **canonical ride path is the lens**, not a collection of independent global panels. Two linked axes: distance-along-path (a cursor hovered/scrubbed on the bottom profile graph, mirrored as a marker on the map path) and world-time (the existing version flipbook, which re-samples the profile so you watch the ride change as the world ages). Concretely: (1) per-species tile colors — each `species_*_density` layer gets a distinct, lineage-coherent hue rendered as RGBA (density→alpha, absent→transparent), replacing the single green ramp that made all species indistinguishable; (2) the ride path + ±50 m sampling strip drawn on the interactive map; (3) the four global timeline charts (population/weather/diversity/speciation) replaced by one version-aware ride-profile graph fed by a new `/api/world/<v>/ride_profile` endpoint that samples live (the old `/api/ride/experience` served a stale file); (4) map↔graph cursor linking; (5) `--snapshot-interval`/`--summary-interval` CLI flags on create/advance for purpose-built runs.

**Why**: The previous UI was "too much and not enough" — four disconnected global-aggregate charts (starved to 2–4 points on framework worlds, since they read orchestrator-written `tick_summary`) that could never answer a question about a *place*. The ride is the project's whole point, and almost all the machinery already existed in `ride_experience.py` (A* path, perpendicular-strip sampler, across-version comparison) but was offline matplotlib + unused API routes. The redesign surfaces and links it. The ride-strip stays neutral black (species story lives in the profile graph, not the map strip — keeps the map uncluttered). Snapshot cadence is a *run* parameter, not a tool parameter: the same inspector consumes a 20×8 yr "month-of-riding" run (how much does the world change between rides?) or a ×100 yr "biome migration" run — set via the new flags.

**Cleanup**: Removed the dead Flask `/api/timeline/*` routes and the frontend chart-builder JS they fed. Kept the `WorldQuery.get_*_timeline` methods (a Layer-B query surface for global stats that "live elsewhere") and their direct tests. No composite-layer plumbing existed in the webview to remove. Full suite green (325 passed, 17 skipped).

---

## R2026-06-21 — Genome moisture-niche redistribution onto the climate manifold

**Decision**: At ancestor creation, stop taking each archetype's `drought_tolerance` straight from its template. Instead, condition it on the *realized* climate: measure the world's `(temp_norm, drought_stress)` point cloud, and for each archetype's warmth (`1 − frost_tolerance`) set `drought_tolerance` to the moisture the world actually pairs with that temperature (mean drought_stress of nearby cells, plus jitter scaled to the local spread, clamped to the achievable temp range). Temperature identity (`frost_tolerance`) and all structural traits are kept from the template.

**Why**: This world's temperature and precipitation are anticorrelated (~−0.50, driven by elevation: lapse-rate cooling + orographic wetting), so the achievable climate is a ~1D diagonal — warm-dry ↔ cold-wet. The hand-placed archetypes put ~half the roster in the unreachable warm-AND-wet (and cold-AND-dry) corners, where suitability is ~0 everywhere, so 7 of 14 species sat permanently at the refugium floor (density 1–2). That halved effective diversity (rider sees ~7 biome players, not 14) and wasted ~2× tick compute on dead species. Conditioning the moisture trait on the measured manifold guarantees every niche lands on reachable climate; verified that all 14 species now establish at seeding (was 7), and a cross-cycle suitability test asserts no ancestor is stranded.

**Why moisture-only**: stranding is purely a moisture-pairing error (a "warm-wet" archetype is fine being warm; it just can't also be wet here). Remapping only `drought_tolerance` is the minimal fix and preserves the temperature semantics encoded in archetype names/tests (e.g. alpine_cushion stays cold, valley_hardwood stays tall) — though a "warm-wet" archetype now reads as warm-dry, which is the cost of the world's climate. Visual/phenotype mutation and a dormancy-rescue mechanic are the separate, later levers for ongoing novelty; this fixes the *starting roster*.

**Tests**: `test_ancestor_traits_close_to_templates` now exempts `drought_tolerance` (world-derived, not template-bound). New `test_no_ancestor_stranded_off_manifold` samples the aperiodic climate across a wide year×season window and requires every ancestor's peak suitability to clear a healthy bar. `test_zero_growth_suppresses_species` switched from a vs-control comparison (confounded by a growing control overshooting and shedding to competition under the new dynamics) to the robust self-decline invariant.

**Pending**: regenerate `worlds/baseline_seed42` (full 800yr, overnight) to confirm long-run persistence of the now-viable species under competition; the equilibrium/long-run behavior is not yet verified, only seeding + niche viability.

---

## R2026-06-21 — Allee establishment threshold (toggleable stability lever)

**Decision**: Add an optional Allee effect to the ecology tick: an `allee_theta` knob (default `0.0` = off) that, when positive, scales *colonizing* (positive) growth by `density / (density + allee_theta)`. Negative die-back growth is left untouched. It is a plain toggle like `refugium_floor` / `enable_extinction` — off by default so it changes no existing run, baseline, or test.

**Why**: We want the world to *accumulate legible history* when ridden over a long time — fire scars and other shocks should leave marks a careful observer is rewarded for noticing — without destabilizing the calm climate-driven background. Reading the growth rule clarified the real levers:
- Growth is `density × growth_rate × logistic × 0.25`, i.e. **proportional to current density**, so a cell at true zero already recovers only by dispersal (a spatial front exists). Fires healed *fast* mainly because `kill=0.9` left 10% residual that regrows logistically in place.
- `refugium_floor` is a **global-extinction lifeline** (seeds the single best cell when a species' *total* falls below the threshold), not a local-recovery mechanism — so "spatial refugium floor" was the wrong handle.
- The mechanism that actually makes scars **persist** and recovery **spatial** is positive density dependence: without it, a single stray propagule regrows exponentially and refill is inevitable; with an Allee threshold, sparse cells can't self-bootstrap, so recolonization needs sustained dispersal from a real source — scars refill slowly from their edges and some don't refill at all.
